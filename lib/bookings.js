...
const isZoomRegistrantDailyLimit = (error) =>
  error?.status === 429 &&
  /exceeded the daily rate limit.*add meeting registrant/i.test(
    error?.body?.message || ''
  )
...
export const reserveLiveSession = async ({ userId, liveSessionId }) => {
  ...
  const unlimited = hasActiveUnlimitedAccess(user)
  ...
  const booking = await prisma.$transaction(async (tx) => {
    ...
    // Scholarship / unlimited access skips the credit system entirely —
    // no grant is consumed, no CreditTransaction is written.
  ...
  try {
    ...
    const registrantDailyLimit = isZoomRegistrantDailyLimit(err)
    ...
    throw new Error(
      registrantDailyLimit
        ? 'ZOOM_REGISTRANT_DAILY_LIMIT'
